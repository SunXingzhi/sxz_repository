...
> Note: 注意`IVT->self`, `IVT->entry`, `BOOT Data->start`三者的"位置关系", self表示IVT自身在内存的绝对地址, 不包括前面的1kb的SD卡预留空间, entry表示用户程序(.imx file 除去 IVT+DCD+BOOT Data后的用户程序)的地址, 而BOOT Data's start 则表示的是整个镜像文件在内存(DDR)的起始地址.对于uboot程序而言, 这里的`&BootData->start==&`
